...
const WinPopout: React.FC<WinPopoutProps> = ({ show, onAnimationEnd, steps, time }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 font-robotoMono">
            <div className="bg-green-500 rounded-lg p-6 flex flex-col items-center relative w-full max-w-sm mx-4">
                ...
                <h2 className="text-white text-2xl font-bold mb-2 text-center">Congratulations, you got it!</h2>
                <p className="text-white text-lg text-center">Steps: {steps}</p>
                <p className="text-white text-lg text-center">Time: {time}</p>
            </div>
        </div>
    );
};
